...
public class CaptureManager {
    ...
    //isSingle true 只识别一次 false  持续识别
    public CaptureManager(Activity activity, DecoratedBarcodeView barcodeView, boolean isSingle) {
    ...
    //isSingle true 只识别一次 false  持续识别
    public CaptureManager(Fragment activity, DecoratedBarcodeView barcodeView, boolean isSingle) {
        this.isSingle=isSingle;
        ...
        inactivityTimer = new InactivityTimer(activity.getActivity(), new Runnable() {
            @Override
            public void run() {
                Log.d(TAG, "Finishing due to inactivity");
                finish();
        ...
        beepManager = new BeepManager(activity.getActivity());
    }
    ...
    public void decode(BarcodeCallback callback) {
        if(isSingle){
            barcodeView.decodeSingle(callback); //单次识别
        }else {
